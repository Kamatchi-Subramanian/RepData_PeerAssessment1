...
```{r}
input2$date<- strptime(x = as.character(input2$date), format = "%Y-%m-%d")
df5<-NULL
df5test1<-NULL
df5test2<- NULL
  i1<-1
  k<-test3[i1]
  df5test1<- subset(input2, input2$interval==k)
  c5<-df5test1$date
  c6<-weekdays(c5)
  for (j1 in 1:length(test2)){
  if (c6 == "Sunday"){ 
    df5test1$daytype = "weekend"
  } else if (c6 == "Saturday"){ 
    df5test1$daytype = "weekend"
    }else { df5test1$daytype="weekday"}
  }
